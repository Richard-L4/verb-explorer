...
/**
 * Confirms a returning checkout session. Prefers the purchase row written by
 * the webhook; falls back to Stripe itself (and records the row) when the
 * webhook has not landed yet.
 */
export const confirmCheckout = createServerFn({ method: "POST" })
  .inputValidator((input) => z.object({ sessionId: z.string().min(1) }).parse(input))
  .handler(async ({ data }) => {
    const { getStripe, recordPurchase, purchaseExists } = await import("./payments.server");
    ...
    return { paid: true, recorded: true };
  });

/** Restores access on another device from the email used at checkout. */
export const restorePurchase = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return { found: await purchaseExistsForEmail(data.email) };
  });
